...
def assert_serialization_roundtrip(event: EventBase) -> None:
    """
    ...
    deserialized = event.__class__.model_validate(event_dict)
    
    # Check if the deserialized event is equal to the original
    assert event == deserialized, f"Serialization roundtrip failed for {event.__class__.__name__}"
