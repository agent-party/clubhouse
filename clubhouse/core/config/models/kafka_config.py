...
class KafkaConfig(BaseModel):
    ...
    topic_prefix: str = Field(default="", description="Prefix for all Kafka topics")
    group_id: Optional[str] = Field(default=None, description="Consumer group ID")
    client_id: Optional[str] = Field(
        default=None, description="Client ID for Kafka producer/consumer"
    )
    ...
    def validate_auto_offset_reset(cls, v: str) -> str:
        ...
        if v not in valid_values:
            raise ValueError(
                f"Invalid auto_offset_reset value. Must be one of: {', '.join(valid_values)}"
            )
        return v
