...
class LogHandlerConfig(BaseModel):
    ...
    type: str = Field(..., description="Handler type (console, file, json_file)")

    level: str = Field("INFO", description="Minimum log level to process")

    format: str = Field("json", description="Log format (json or text)")

    # Handler-specific configuration options
    options: Dict[str, Any] = Field(
        default_factory=dict, description="Handler-specific options"
    )
...
class LoggingConfig(BaseModel):
    ...
    include_timestamps: bool = Field(
        True, description="Include timestamps in log entries"
    )

    propagate_context: bool = Field(
        True, description="Propagate context across async boundaries"
    )

    include_traceback: bool = Field(
        True, description="Include traceback information for errors"
    )
    ...
    global_context: Dict[str, Any] = Field(
        default_factory=dict, description="Context to include in all log entries"
    )
